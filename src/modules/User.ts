...
export default class User implements WarrantObject {
  ...
  constructor(userId: string, email?: string, createdAt?: Date) {
    this.userId = userId;
    this.email = email;
  ...
  public static async get(userId: string): Promise<User> {
    try {
      ...
      return new User(response.userId, response.email, response.createdAt);
    } catch (e) {
      throw e;
    }
  }
  ...
  public static async listUsers(
    ...
    try {
      ...
      return response.map(
        (user: User) =>
          new User(user.userId, user.email, new Date(user.createdAt))
      );
    } catch (e) {
      throw e;
    }
  ...
  public static async listUsersForTenant(
    tenantId: string,
    listOptions: ListUserOptions = {}
  ): Promise<User[]> {
    try {
  ...
  public static async assignUserToTenant(
    tenantId: string,
    userId: string,
    role: string
  ): Promise<Warrant> {
  ...
  public static async removeUserFromTenant(
    tenantId: string,
    userId: string,
    role: string
  ): Promise<void> {
  ...
  public async hasPermission(
    ...
    return Authorization.hasPermission({
      ...
      subject: { objectType: ObjectType.User, objectId: this.userId },
      context: context,
    });
  }
