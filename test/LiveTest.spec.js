...
describe.skip("Live Test", function () {
  ...
  it("CRUD users", async function () {
    ...
    let user2 = await this.warrant.User.create({
      userId: "some_id",
    ...
    user2 = await this.warrant.User.update("some_id", {
  ...
  it("CRUD tenants", async function () {
    ...
    let tenant2 = await this.warrant.Tenant.create({
    ...
    tenant2 = await this.warrant.Tenant.update("some_tenant_id", {
      name: "updated_name",
    });
  ...
  it("CRUD roles", async function () {
    const adminRole = await this.warrant.Role.create({
      roleId: "admin",
      name: "Admin",
    ...
    let viewerRole = await this.warrant.Role.create({
      roleId: "viewer",
      name: "Viewer",
    ...
    viewerRole = await this.warrant.Role.update("viewer", {
      name: "Viewer Updated",
  ...
  it("CRUD permissions", async function () {
    ...
    let permission2 = await this.warrant.Permission.create({
      permissionId: "perm2",
      name: "Permission 2",
      description: "Permission with id 2",
    });
    let refetchedPermission = await this.warrant.Permission.get(
      permission2.permissionId
    );
    assert.strictEqual(
      permission2.permissionId,
      refetchedPermission.permissionId
    );
    assert.strictEqual(permission2.name, refetchedPermission.name);
    assert.strictEqual(
      permission2.description,
      refetchedPermission.description
    );

    permission2 = await this.warrant.Permission.update("perm2", {
    ...
    let permissions = await this.warrant.Permission.listPermissions({
      limit: 10,
      page: 1,
    });
    ...
    permissions = await this.warrant.Permission.listPermissions({
      limit: 10,
      page: 1,
    });
  ...
  it("CRUD features", async function () {
    const feature1 = await this.warrant.Feature.create({
      featureId: "new-feature",
    });
    assert.strictEqual(feature1.featureId, "new-feature");

    const feature2 = await this.warrant.Feature.create({
      featureId: "feature-2",
    });
    ...
    let features = await this.warrant.Feature.listFeatures({
      limit: 10,
      page: 1,
    });
  ...
  it("CRUD pricing tiers", async function () {
    const tier1 = await this.warrant.PricingTier.create({
      pricingTierId: "new-tier-1",
    });
    assert.strictEqual(tier1.pricingTierId, "new-tier-1");

    const tier2 = await this.warrant.PricingTier.create({
      pricingTierId: "tier-2",
    });
    const refetchedTier = await this.warrant.PricingTier.get(
      tier2.pricingTierId
    );
    assert.strictEqual(tier2.pricingTierId, refetchedTier.pricingTierId);

    let tiers = await this.warrant.PricingTier.listPricingTiers({
      limit: 10,
      page: 1,
    });
    ...
    tiers = await this.warrant.PricingTier.listPricingTiers({
      limit: 10,
      page: 1,
    });
  ...
  it("batch create users and tenants", async function () {
    const newUsers = [{ userId: "user-1" }, { userId: "user-2" }];
    ...
    const newTenants = [{ tenantId: "tenant-1" }, { tenantId: "tenant-2" }];
  ...
  it("multi-tenancy example", async function () {
    ...
    const tenant2 = await this.warrant.Tenant.create({
      tenantId: "tenant-2",
      name: "Tenant 2",
    });

    let user1Tenants = await this.warrant.Tenant.listTenantsForUser(
      user1.userId,
      { limit: 100, page: 1 }
    );
    let tenant1Users = await this.warrant.User.listUsersForTenant("tenant-1", {
      limit: 100,
      page: 1,
    });
    ...
    await this.warrant.User.assignUserToTenant(
      tenant1.tenantId,
      user1.userId,
      "member"
    );

    user1Tenants = await this.warrant.Tenant.listTenantsForUser(user1.userId, {
      limit: 100,
      page: 1,
    });
    ...
    tenant1Users = await this.warrant.User.listUsersForTenant("tenant-1", {
      limit: 100,
      page: 1,
    });
    ...
    await this.warrant.User.removeUserFromTenant(
      tenant1.tenantId,
      user1.userId,
      "member"
    );
    ...
    tenant1Users = await this.warrant.User.listUsersForTenant(
      tenant1.tenantId,
      { limit: 100, page: 1 }
    );
    assert.strictEqual(tenant1Users.length, 0);
  ...
  it("RBAC example", async function () {
    ...
    const viewerRole = await this.warrant.Role.create({
      roleId: "viewer",
      name: "Viewer",
      description: "The viewer role",
    });
    ...
    // Create permissions
    const createPermission = await this.warrant.Permission.create({
      permissionId: "create-report",
      name: "Create Report",
      description: "Permission to create reports",
    });
    const viewPermission = await this.warrant.Permission.create({
      permissionId: "view-report",
      name: "View Report",
      description: "Permission to view reports",
    });

    let adminUserRoles = await this.warrant.Role.listRolesForUser(
      adminUser.userId,
      { limit: 100, page: 1 }
    );
    let adminRolePermissions =
      await this.warrant.Permission.listPermissionsForRole(adminRole.roleId, {
        limit: 100,
        page: 1,
      });
    ...
    let adminUserHasPermission = await this.warrant.Authorization.hasPermission(
    ...
    await this.warrant.Permission.assignPermissionToRole(
      adminRole.roleId,
      createPermission.permissionId
    );
    await this.warrant.Role.assignRoleToUser(
      adminUser.userId,
      adminRole.roleId
    );
    ...
    adminUserRoles = await this.warrant.Role.listRolesForUser(
      adminUser.userId,
      { limit: 100, page: 1 }
    );
    ...
    adminRolePermissions = await this.warrant.Permission.listPermissionsForRole(
      adminRole.roleId,
      { limit: 100, page: 1 }
    );
    assert.strictEqual(adminRolePermissions.length, 1);
    assert.strictEqual(
      adminRolePermissions[0].permissionId,
      createPermission.permissionId
    );

    await this.warrant.Permission.removePermissionFromRole(
      adminRole.roleId,
      createPermission.permissionId
    );
    await this.warrant.Role.removeRoleFromUser(
      adminUser.userId,
      adminRole.roleId
    );
    ...
    let viewerUserPermissions =
      await this.warrant.Permission.listPermissionsForUser(viewerUser.userId, {
        limit: 100,
        page: 1,
      });
    assert.strictEqual(viewerUserPermissions.length, 0);

    await this.warrant.Permission.assignPermissionToUser(
      viewerUser.userId,
      viewPermission.permissionId
    );

    viewerUserHasPermission = await this.warrant.Authorization.hasPermission({
      permissionId: "view-report",
      subject: viewerUser,
    });
    assert.strictEqual(viewerUserHasPermission, true);

    viewerUserPermissions =
      await this.warrant.Permission.listPermissionsForUser(viewerUser.userId, {
        limit: 100,
        page: 1,
      });
    ...
    assert.strictEqual(
      viewerUserPermissions[0].permissionId,
      viewPermission.permissionId
    );

    await this.warrant.Permission.removePermissionFromUser(
      viewerUser.userId,
      viewPermission.permissionId
    );
  ...
  it("pricing tiers, features, and users example", async function () {
    ...
    const freeTier = await this.warrant.PricingTier.create({
      pricingTierId: "free",
    });
    const paidTier = await this.warrant.PricingTier.create({
      pricingTierId: "paid",
    });

    // Create features
    const customFeature = await this.warrant.Feature.create({
    ...
    let paidUserHasFeature = await this.warrant.Authorization.hasFeature({
      featureId: "custom-feature",
      subject: paidUser,
    });
    assert.strictEqual(paidUserHasFeature, false);

    let paidUserFeatures = await this.warrant.Feature.listFeaturesForUser(
      paidUser.userId,
      { limit: 100, page: 1 }
    );
    assert.strictEqual(paidUserFeatures.length, 0);

    await this.warrant.Feature.assignFeatureToUser(
      paidUser.userId,
      customFeature.featureId
    );

    paidUserHasFeature = await this.warrant.Authorization.hasFeature({
      featureId: "custom-feature",
      subject: paidUser,
    });
    assert.strictEqual(paidUserHasFeature, true);

    paidUserFeatures = await this.warrant.Feature.listFeaturesForUser(
      paidUser.userId,
      { limit: 100, page: 1 }
    );
    ...
    await this.warrant.Feature.removeFeatureFromUser(
      paidUser.userId,
      customFeature.featureId
    );
    ...
    let freeUserHasFeature = await this.warrant.Authorization.hasFeature({
      featureId: "feature-1",
      subject: freeUser,
    });
    assert.strictEqual(freeUserHasFeature, false);

    let freeTierFeatures =
      await this.warrant.Feature.listFeaturesForPricingTier(
        freeTier.pricingTierId,
        { limit: 100, page: 1 }
      );
    assert.strictEqual(freeTierFeatures.length, 0);

    let freeUserTiers = await this.warrant.PricingTier.listPricingTiersForUser(
      freeUser.userId,
      { limit: 100, page: 1 }
    );
    assert.strictEqual(freeUserTiers.length, 0);

    await this.warrant.Feature.assignFeatureToPricingTier(
      freeTier.pricingTierId,
      feature1.featureId
    );
    await this.warrant.PricingTier.assignPricingTierToUser(
      freeUser.userId,
      freeTier.pricingTierId
    );

    freeUserHasFeature = await this.warrant.Authorization.hasFeature({
      featureId: "feature-1",
      subject: freeUser,
    });
    assert.strictEqual(freeUserHasFeature, true);

    freeTierFeatures = await this.warrant.Feature.listFeaturesForPricingTier(
      freeTier.pricingTierId,
      { limit: 100, page: 1 }
    );
    ...
    freeUserTiers = await this.warrant.PricingTier.listPricingTiersForUser(
      freeUser.userId,
      { limit: 100, page: 1 }
    );
    ...
    await this.warrant.Feature.removeFeatureFromPricingTier(
      freeTier.pricingTierId,
      feature1.featureId
    );
    await this.warrant.PricingTier.removePricingTierFromUser(
      freeUser.userId,
      freeTier.pricingTierId
    );
  ...
  it("pricing tiers, features, and tenants example", async function () {
    ...
    let paidTenantHasFeature = await this.warrant.Authorization.hasFeature({
      featureId: "custom-feature",
      subject: paidTenant,
    });
    assert.strictEqual(paidTenantHasFeature, false);

    let paidTenantFeatures = await this.warrant.Feature.listFeaturesForTenant(
      paidTenant.tenantId,
      { limit: 100, page: 1 }
    );
    assert.strictEqual(paidTenantFeatures.length, 0);

    await this.warrant.Feature.assignFeatureToTenant(
      paidTenant.tenantId,
      customFeature.featureId
    );

    paidTenantHasFeature = await this.warrant.Authorization.hasFeature({
      featureId: "custom-feature",
      subject: paidTenant,
    });
    assert.strictEqual(paidTenantHasFeature, true);

    paidTenantFeatures = await this.warrant.Feature.listFeaturesForTenant(
      paidTenant.tenantId,
      { limit: 100, page: 1 }
    );
    ...
    await this.warrant.Feature.removeFeatureFromTenant(
      paidTenant.tenantId,
      customFeature.featureId
    );
    ...
    let freeTenantHasFeature = await this.warrant.Authorization.hasFeature({
      featureId: "feature-1",
      subject: freeTenant,
    });
    ...
    let freeTenantTiers =
      await this.warrant.PricingTier.listPricingTiersForTenant(
        freeTenant.tenantId,
        { limit: 100, page: 1 }
      );
    ...
    await this.warrant.PricingTier.assignPricingTierToTenant(
      freeTenant.tenantId,
      freeTier.pricingTierId
    );

    freeTenantHasFeature = await this.warrant.Authorization.hasFeature({
      featureId: "feature-1",
      subject: freeTenant,
    });
    ...
    freeTenantTiers = await this.warrant.PricingTier.listPricingTiersForTenant(
      freeTenant.tenantId,
      { limit: 100, page: 1 }
    );
    ...
    await this.warrant.PricingTier.removePricingTierFromTenant(
      freeTenant.tenantId,
      freeTier.pricingTierId
    );
  ...
  it("sessions", async function () {
    ...
    const userAuthzSession =
      await this.warrant.Session.createAuthorizationSession({
        userId: user.userId,
      });
    assert(userAuthzSession);

    const userSelfServicDashboardUrl =
  ...
  it("warrants", async function () {
    ...
    const newPermission = await this.warrant.Permission.create({
      permissionId: "perm1",
      name: "Permission 1",
      description: "Permission with id 1",
    });
